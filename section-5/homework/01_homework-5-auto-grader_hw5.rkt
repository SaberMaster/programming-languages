...
(define (eval-under-env e env)
  (cond [(var? e) 
        ...
        ;;    (if (closure? v1)
        ;;        (eval-under-env (mlet (fun-formal v1) v2 (fun-body v1)) (if (fun-nameopt v1)
        ;;                                                                    (cons (cons (fun-nameopt v1) v1) env) env))
         ...
         (let ((v1 (call-funexp e))
           ...
           (if (closure? v1)
               (let ((v3 (closure-fun v1))
                     (env (closure-env v1)))
                 (eval-under-env (mlet (fun-formal v3) v2 (fun-body v3)) (if (fun-nameopt v3)
                                                                             (cons (cons (fun-nameopt v3) v3) env) env)))
             (error "MUPL call applied to non-closure")))]
